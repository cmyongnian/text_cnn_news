...
from utils import (
    ...
    update_results_csv,
    upsert_section,
    plot_confusion_matrix,  # 增加
    save_error_examples     # 增加
)
...
def main():
    ...
    pred_path = output_dir / f"preds_{args.exp}.json"

    with open(pred_path, "w", encoding="utf-8") as f:
        json.dump(test_metrics["preds"], f, ensure_ascii=False)
        texts = [x["sentence"] for x in test_raw]
    ...
    save_error_examples(
        ...
        id2desc=id2desc,
        save_path = str(report_path).replace(".txt", "_errors.txt"),
        k=20
    )
    ...
    test_metrics["preds"],
    id2desc,
    str(fig_path).replace(".png", "_cm.png")
    )
