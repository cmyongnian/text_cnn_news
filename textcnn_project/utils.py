...
def plot_confusion_matrix(y_true, y_pred, id2desc, save_path):
...
def save_error_examples(texts, labels, preds, id2desc, save_path, k=20):
    """
    保存模型错误预测样本（Error Analysis）
    """
    ...
    wrong_samples = wrong_samples[:k]

    lines = []
    for item in wrong_samples:
        true_label = id2desc[item["true"]]
        pred_label = id2desc[item["pred"]]
